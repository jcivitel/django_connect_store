...
def import_putty_connections(request):
    if request.method == 'POST' and request.FILES.get('putty_file'):
        ...
        sessions = re.findall(r'\[HKEY_CURRENT_USER\\Software\\SimonTatham\\PuTTY\\Sessions\\([^\]]+)\]([^\[]+)',
                              putty_file)
        ...
        for session_name, session_data in sessions:
            ...
            if hostname and port and protocol:
                ...
                proto, _ = Proto.objects.get_or_create(name=protocol.upper(), defaults={'pref': protocol.lower(),
                                                                                        'url_scheme': protocol.lower()})
        ...
        messages.success(request, f'Successfully imported {imported_count} connections.')
        return redirect('dashboard')
...
def download_ps_script(request):
    file_path = os.path.join(settings.BASE_DIR, 'django_connect_backend', 'static', 'export_putty_sessions.ps1')
    return FileResponse(open(file_path, 'rb'), as_attachment=True, filename='export_putty_sessions.ps1')
